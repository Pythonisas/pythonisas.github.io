...
** TODO Practica 3.9 - Invasion Alienigena IV: Disparando Balas
:PROPERTIES:
...
Tu nave se mueve... pero es inofensiva. Necesita armas.

Al pulsar ESPACIO, la nave debera disparar proyectiles que suban por la pantalla. Suena simple, pero hay tres problemas de ingenieria escondidos:

1. *La fuga de memoria* — Cada disparo crea un objeto nuevo. Si no los destruyes cuando salen de pantalla, el juego acumula cientos de balas invisibles que consumen RAM. ¿Como detectas y eliminas objetos que ya no sirven?

2. *El dilema de la coleccion* — Necesitas guardar todas las balas activas en algun sitio, actualizarlas todas cada frame, y dibujarlas todas. ¿Como gestionas una coleccion que crece y encoge dinamicamente?

3. *El recurso limitado* — Si el jugador puede disparar sin limite, el juego pierde toda la tension. ¿Como limitas las balas en pantalla para que apuntar bien sea obligatorio?
...
- =pygame.sprite.Sprite= — clase base para objetos que Pygame sabe agrupar. Tu =Bala= heredara de ella
- =pygame.sprite.Group= — coleccion de sprites con superpoderes: =.update()= llama a =actualizar()= de todos, =.sprites()= los lista, =.add()= y =.remove()= los gestionan
- =pygame.Rect(x, y, ancho, alto)= — crea un rectangulo sin imagen (la bala no necesita un .bmp)
- =pygame.draw.rect(superficie, color, rect)= — dibuja un rectangulo solido en pantalla
...
- Se dibuja como un rectangulo (sin imagen)

*Pistas tecnicas:*
- La bala es un nuevo fichero (=bala.py=) con una clase que hereda de =Sprite=
- Necesita =actualizar()= (mover hacia arriba) y =dibujar_bala()= (pintar el rectangulo)
- La posicion inicial: =self.rect.midtop = ia_juego.nave.rect.midtop=
- Recuerda el patron float: =self.y = float(self.rect.y)= para velocidades fraccionarias
- Centraliza los ajustes (velocidad, tamaño, color) en =ajustes.py=

---

*** Desafio 2: La fuga de memoria

Implementa el disparo y ejecuta el juego. Ahora añade esta linea en tu game loop, despues de actualizar las balas:

#+begin_src python
print(len(self.balas))
#+end_src

Dispara 10 veces y observa la consola. ¿El numero baja cuando las balas salen de pantalla? Si no baja, tienes una *fuga de memoria*: las balas siguen vivas aunque no se vean.

*Tu mision:* haz que el contador vuelva a 0 despues de que las balas salgan por arriba.

*Pista:* =bala.rect.bottom <= 0= indica que la bala ha salido. Pero cuidado: no puedes modificar una coleccion mientras la recorres. ¿Como lo resuelves?

*IMPORTANTE:* una vez que verifiques que funciona, *elimina el print()*. Escribir en consola cada frame ralentiza el juego mas que dibujar graficos.

---

*** Desafio 3: Energia limitada

Sin limite, el jugador puede llenar la pantalla de balas manteniendo ESPACIO pulsado. Eso destruye la tension del juego.

*Tu mision:* limita a 3 balas simultaneas en pantalla. Si ya hay 3, pulsar ESPACIO no hace nada hasta que alguna desaparezca.

*Pregunta de diseño:* ¿donde colocas este limite? ¿En la clase =Bala=, en el game loop, o en =Ajustes=? Justifica tu decision.

---

*** Desafio 4: Codigo limpio

Tu game loop se ha complicado. Tienes logica de eventos, actualizacion de nave, actualizacion de balas, limpieza de balas, y dibujo — todo mezclado.

*Tu mision:* extrae la logica de balas a un metodo privado =_actualizar_balas()=. El game loop deberia quedar en 5-6 lineas legibles.
...
| *Nivel*    | *Descripcion*                                                                          |
|------------+----------------------------------------------------------------------------------------|
| *Maestro*  | Los 4 desafios resueltos. Codigo modular. Sabe explicar por que se usa =.copy()= para borrar balas y por que =float= para la posicion. Si uso IA: incluye prompt y demuestra comprension |
| *Aprendiz* | Disparo funciona + limpieza de memoria + limite de balas. Algun detalle menor pendiente (game loop no del todo limpio, o ajustes hardcodeados en vez de en =Ajustes=) |
| *Recluta*  | El disparo funciona pero sin limpieza de memoria o sin limite de balas. O el codigo funciona pero no sabe explicar por que |

*Sobre el uso de IA:* puedes usar ChatGPT, Copilot, Claude o cualquier herramienta. Pero si lo haces, incluye en un comentario al inicio del fichero: (1) el prompt que usaste, y (2) explica en tus palabras por que se usa =self.balas.copy()= en vez de =self.balas= para el bucle de limpieza.
...
| *Fuga de memoria*                 | Objetos que ya no se ven pero siguen consumiendo recursos             |
| *Iteracion segura con .copy()*    | No puedes borrar de una coleccion mientras la recorres                |
| *Limitar recursos del jugador*    | =balas_permitidas= obliga a apuntar bien — diseño de juego           |
...
** TODO Practica 3.10 - Invasion Alienigena V: Alienigenas!
:PROPERTIES:
...
*** Paso 1 -- Crear la clase =Alienigena=
...
**** 2.2 Metodo =_crear_flota()= (version minima)
...
**** 3.1 El metodo auxiliar =_crear_alienigena()=
...
*** Paso 4 -- Añadir filas (la flota completa)
...
#+begin_quote
Basado en el Capitulo 13 del libro /Python Crash Course, 3rd Edition/ -- Eric Matthes
#+end_quote
...
*** Paso 2 -- Detectar los bordes de la pantalla
...
*** Paso 3 -- Cambiar direccion y bajar
...
*** Paso 4 -- Refactorizar: =_actualizar_alienigenas()=
...
#+begin_quote
Los ejercicios 13-3 y 13-4 son los /TRY IT YOURSELF/ del libro. Intentarlos es la mejor forma de consolidar lo aprendido.
#+end_quote
...
- Usar un truco de /balas gigantes/ para testear mas rapido
...
*** Paso 1 -- Refactorizar: =_verificar_colisiones_bala_alienigena()=
...
*** Paso 2 -- Ajustar la velocidad de las balas
...
*** Paso 3 -- Truco: balas gigantes para testear
...
[[/images/balas-gigantes-test.png]]

#+begin_quote
/Recuerda restaurar =self.ancho_bala = 3= cuando termines de testear. No entregues el juego con balas gigantes!/
#+end_quote

Tambien puedes combinar con el primer argumento =False= de =groupcollide()= para crear una /bala superpoderosa/ que atraviesa toda la pantalla sin desaparecer:
...
*** Paso 4 -- Detectar colision alien-nave
...
Recorre el grupo y devuelve el primer alienigena que colisiona con la nave. Si ninguno colisiona, devuelve =None=. Como =None= es /falsy/, el =if= solo se activa cuando hay colision.
...
*** Paso 5 -- Detectar alienigenas que llegan al fondo
...
*** Paso 6 -- Crear =_nave_alcanzada()= (preparando el terreno)
...
#+begin_quote
El ejercicio 13-5 es el /TRY IT YOURSELF/ del libro para esta seccion. Intentarlo es la mejor forma de consolidar lo aprendido.
#+end_quote
